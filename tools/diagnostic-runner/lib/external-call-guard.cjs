 ...
 * - better-sqlite3 요청을 tools 로컬 Node-ABI 사본으로 리다이렉트한다(루트 사본은
 *   electron:build에 의해 Electron ABI로 뒤집힌다).
...
try {
  ...
  if (_fs.existsSync(_envFile)) {
    ...
    if (_injected.OSOO_TOOLS_SQLITE_DIR) {
      _toolsSqliteDir = String(_injected.OSOO_TOOLS_SQLITE_DIR);
    }
  }
} catch (_e) { /* 주입 실패 시 기존 env에 의존한다 */ }
...
// 2) better-sqlite3 리다이렉트: tools 로컬 Node-ABI 사본으로 로드를 고정한다.
...
Module._load = function patchedLoad(request, parent, isMain) {
  if (request === 'better-sqlite3' && _toolsSqliteDir) {
...
// SDK 로드 추적: patchedLoad에서 original을 감싸 기록한다(부팅 증거와 함께 리포트에 남긴다).
