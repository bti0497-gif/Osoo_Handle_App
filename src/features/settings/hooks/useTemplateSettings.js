...
export const useTemplateSettings = ({ showAlert, reloadSettings } = {}) => {
    ...
    const handleOpenLocalFolder = async (target) => {
        try {
            const electronOpenFolder = window.electronAPI?.openFolder;
            if (typeof electronOpenFolder === 'function') {
                const result = await electronOpenFolder(target);
                if (!result?.ok || !result?.path) {
                    throw new Error(result?.error || 'Windows 탐색기를 실행하지 못했습니다.');
                }
                return;
            }
