...
- **런타임 설정 경로는 릴리즈 계약입니다. 앱은 `%APPDATA%\Osoo_Handle_App\config`를 기본 경로로 사용하고, 기존 현장 호환을 위해 `%APPDATA%\wastewater-treatment-plant\config`를 fallback으로 읽어야 합니다.**
- **설치/프로비저닝 스크립트는 `.env.local`, `google-key.json`, `bigquery-service-account.json`, `firebase-service-account.json`을 반드시 `%APPDATA%\Osoo_Handle_App\config`에 설치해야 하며, 호환을 위해 `%APPDATA%\wastewater-treatment-plant\config`에도 함께 복사해야 합니다.**
- **자격증명 파일은 `electron-builder` 패키지 본문에 포함하지 않습니다. 반드시 설치/프로비저닝 단계에서 사용자 AppData로 배치하고, `npm run validate`와 `npm run validate:asar`로 경로 계약과 자격증명 미포함 여부를 확인합니다.**
- 런타임 설정 경로, 설치 스크립트, 통합 설치 스크립트 중 하나를 수정한 경우 릴리즈 전에 `scripts/provision-runtime-config.*`, `scripts/install-with-provisioning.ps1`, `scripts/build-integrated-installer.ps1`, `server/config/runtimeConfig.cjs`가 같은 경로 계약을 따르는지 확인합니다.
