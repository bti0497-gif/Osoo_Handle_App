...
module.exports = function (db) {
    router.get('/api/kits/history', (req, res) => {
        try {
            ...
            const allRecords = db.prepare(`SELECT * FROM kit_logs ${whereSql} ORDER BY date ASC`).all(...params);
            const hasOlder = fromDate
                ? Boolean(site_id
                    ? db.prepare('SELECT 1 FROM kit_logs WHERE site_id = ? AND date < ? LIMIT 1').get(String(site_id), String(fromDate))
                    : db.prepare('SELECT 1 FROM kit_logs WHERE date < ? LIMIT 1').get(String(fromDate)))
                : false;
