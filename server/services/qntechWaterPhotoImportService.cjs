...
async function saveProjectPhotos({ baseUrl, cookieJar, projects, date, baseDir, configuredPhotoRoot }) {
  ...
  for (const project of projects) {
    for (const file of project.files || []) {
      ...
      if (selectedFiles.some((existing) => existing.itemName === matchedItem)) continue;
      selectedFiles.push({
  ...
  for (const file of selectedFiles) {
    const baseName = `${date}_${sanitizeSegment(file.itemName)}`;
    ...
    const targetPath = path.join(photoDir, `${baseName}${ext}`);
