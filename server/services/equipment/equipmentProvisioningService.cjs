...
const COMMON_DEFS = () => [
  ...
  ...unitCards(null, '포기조', '브로아류', '포기조브로아', { managementNoBase: 'M-105', model: '루츠 블로어', power: '7.5 kW', location: '송풍기실', accessory: '공기 여과기 포함', notes: '기본 2대 · 추가 시 C, D, E...', statusByUnit: { A: '수리 중' } }),
  ...unitCards(null, '포기조', '펌프류', '내부반송펌프', { managementNoBase: 'M-106', model: '수중펌프', power: '2.2 kW', notes: '기본 2대 · 추가 시 C, D, E...', statusByUnit: { A: '점검 필요' } }),
  ...
  ...unitCards(null, '방류조', '펌프류', '방류펌프', { managementNoBase: 'M-114', model: '수중펌프', power: '2.2 kW', notes: '기본 2대 · 추가 시 C, D, E...' }),
  ...unitCards(null, '방류조', '펌프류', '중수펌프', { managementNoBase: 'M-115', model: '수중펌프', power: '1.5 kW', notes: '기본 2대 · 추가 시 C, D, E...' }),
  ...unitCards(null, '방류조', '펌프류', '역세펌프', { managementNoBase: 'M-116', model: '수중펌프', power: '1.5 kW', notes: '여과기 역세용 · 기본 2대' }),
  card('M-117', '방류조', '여과기', '여과기', { model: '모래여과기', accessory: '부속 펌프·밸브 포함' }),
  card('M-118', '방류조', '소독기류', 'UV 소독기', { model: '관류식 자외선 소독기', specification: '램프 4식', power: '0.8 kW', accessory: '램프·안정기 포함' }),
];

const A2O_ONLY_DEFS = () => [
  card('M-107', '침전조', '감속기', '감속기', { model: '중심구동 스크래퍼용', power: '0.75 kW', accessory: '스크래퍼 포함' }),
  ...unitCards(null, '침전조', '펌프류', '외부반송펌프', { managementNoBase: 'M-108', model: '수중펌프', power: '2.2 kW', notes: '기본 2대 · 추가 시 C, D, E...' }),
  card('FLT-104', '침전조', '유량계', '외부반송유량계', { model: '전자유량계' }),
  card('M-109', '응집침전조', '감속기', '감속기', { model: '중심구동 스크래퍼용', power: '0.75 kW', accessory: '스크래퍼 포함' }),
  card('M-110', '응집침전조', '탱크류', '응집제탱크', { model: 'PE 저장탱크', unit: '개', location: '약품실' }),
  card('M-111', '응집침전조', '약품펌프류', '응집제 주입펌프', { model: '정량 다이어프램 펌프', power: '0.1 kW', location: '약품실', notes: '예비 포함 시 추가 등록' }),
  card('M-112', '응집침전조', '탱크류', '폴리머탱크', { model: 'PE 저장탱크', unit: '개', location: '약품실', accessory: '교반기 포함' }),
  card('M-113', '응집침전조', '약품펌프류', '폴리머 주입펌프', { model: '정량 다이어프램 펌프', power: '0.1 kW', location: '약품실', notes: '예비 포함 시 추가 등록' }),
];

const MBR_ONLY_DEFS = () => [
  ...unitCards(null, '막분리조', '막분리', '막모듈', { managementNoBase: 'M-119', model: '침지형 평막 모듈', specification: '0.1 um 평막', unit: '식', accessory: '막 지지 프레임 포함', notes: '기본 4식 · 증설 시 추가 등록' }, UNITS4),
  ...unitCards(null, '막분리조', '펌프류', '흡인펌프', { managementNoBase: 'M-120', model: '자흡식 원심펌프', power: '1.5 kW', notes: '교대 운전 2대 · 추가 시 C, D, E...' }),
  card('M-121', '막분리조', '펌프류', '막세척펌프', { model: '정량 다이어프램 펌프', power: '0.4 kW', accessory: 'CIP 약액 라인 포함', notes: '화학세척(CIP)용' }),
