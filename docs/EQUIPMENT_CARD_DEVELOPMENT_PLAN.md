...
- 카탈로그 구성(공법별 자동 필터): 펌프류 / 교반기류(유량조정·무산소·혐기·포기·방류) / 브로아류(포기·교반·급기팬·배기팬) / 스크린류 / 계측기류(DO·PH·MLSS) / 유량계(파샬플롬·유입·내부반송·외부반송·중수·응집이송·막분리) / 감속기 / 탱크류 / 약품펌프류 / 막분리(MBR) / 소독기류 / 여과기
...
| 설비 | 접두어 | 예 |
|---|---|---|
| 기계설비(펌프·교반기·브로아·스크린·탱크·감속기 등) | `M-` + 공정 구간 | M-101 / M-301 / M-501 |
| 유량계 | `FLT-` | FLT-101 |
| 수위계측기 | `LIT-` | LIT-101 |
| 계측기(종류별, 계측기명에서 추출) | `DO-` / `PH-` / `MLSS-` | DO-101 |

- 기계설비 `M-` 번호는 공정 구간별로 독립 채번한다: 침사조·유량조정조 `M-1xx`, 혐기조·무산소조 `M-2xx`, 포기조·막분리조 `M-3xx`, 침전조·응집침전조 `M-4xx`, 여과조·소독조·방류조 `M-5xx`.
- 유량계·수위계·계측기는 접두어별 독립 채번하며 기존 최대값 + 1, 새 접두어는 101부터 시작한다.
...
- 시드(기본 장비)도 동일한 공정 구간 규칙으로 번호가 매겨진다(M-101~, M-301~, M-401~, M-501~, FLT-101~, DO-101, PH-101...).
