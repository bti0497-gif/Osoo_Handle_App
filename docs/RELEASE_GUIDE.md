...
새 통합 설치본은 정기 릴리즈마다 다시 만들지 않습니다. 보관된 최신 검증 통합 설치본으로 최초 설치한 뒤
자동업데이트를 적용합니다. GitHub의 일반 `Setup` 파일은
기존 AppData 설정을 유지하는 업데이트 전용이며, 빈 PC의 최초 설치용으로 사용하지 않습니다.

정기 자동업데이트 릴리즈는 다음 명령만 사용합니다.

```powershell
npm run release:safe
```

이 명령은 업데이트용 설치본만 생성·검증하며 통합 설치본을 만들거나 덮어쓰지 않습니다.
자격증명 변경이나 재해복구 때문에 통합 설치본을 갱신해야 할 때만 위의 수동 명령을 별도로 실행합니다.

Windows 7 32비트 호환판은 `legacy/win7-ia32` 브랜치에서 별도 버전으로 관리하며 자동업데이트 대상이 아닙니다.
