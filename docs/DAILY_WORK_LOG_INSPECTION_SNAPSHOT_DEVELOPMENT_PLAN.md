...
날씨·기온 스냅샷에는 다음 근거도 함께 보존한다.

- 처리장 좌표와 좌표 출처, 공급자, 상품·예보구역 또는 격자
- 발표·조회·유효 기준시각, KST 시간 범위, 관측/예보 구분
- 기온 집계 방식과 유효 표본 수, 자동 원값과 사용자 수정값·사유
- 날씨 코드·표시문구와 `하루 중 가장 심한 상태`를 고른 규칙 버전

결측값은 `null`로 유지하며 빈 값이나 `null`을 `0도` 또는 `맑음`으로 변환하지 않는다. 코드 숫자의 크기로 날씨 심각도를 판단하지 않고 명시적인 우선순위표를 사용한다. 임시 조회 캐시와 확정된 일지 스냅샷은 분리하며, 좌표·공급자·예보 갱신 때문에 확정된 과거 값이 자동 변경되지 않게 한다.
...
6. UI가 읽은 revision과 저장 시 최신 revision이 다르면 `409`로 충돌을 알리고 사용자의 편집값을 유지한다.
7. 저장은 헤더·값·점검항목·장비근거·최신 revision 갱신을 단일 트랜잭션으로 처리한다.
8. 재시도용 요청 ID 또는 revision UUID로 중복 저장을 막고, 같은 ID에 다른 내용이 오면 거부한다.
9. 장비명·관리번호·상태와 판정 근거는 당시 값의 스냅샷을 저장하여 장비 개명이나 다음 날 수리 후에도 과거 판정을 유지한다.
10. 사진 바이너리는 스냅샷 JSON에 복제하지 않고 기존 장비사진·업무사진 식별자를 증빙으로 연결한다.
